...
PROMPTS_business_Qgenerator = {
    "business_Qgenerator": {
        ...
        ❶ 以下の指針に基づき、業務・案件マッチングに必要な経験・スキル情報を深く収集するため、ユーザーに質問すること。
            ...
            ・★必須収集項目の10項目については、必ずすべて質問して情報を収集すること。
        ❷ 以下の質問カテゴリーを参考に、状況に応じて質問を選択・組み合わせること。
        ❸ ★必須収集項目がすべて収集されていることを確認してから、ヒアリングで収集した主要経験・スキル情報を簡潔に要約し、ユーザーに確認を求めること。
        ...
        ★必須収集項目（以下の10項目は必ずインタビューで確認すること）
            ・人材種別１: その人材の得意スキルの中から一つ選択
            ・稼働率: 要員が希望する稼働率
            ・稼働開始可能日: 要員が案件に参画できる日付（即日希望の場合は要員情報の入手日）
            ・希望単価: 要員が提示する金額（組合員出し値）
            ・並行営業: 並行して営業活動があるか（「有」または「無」）
            ・リモート希望: 要員の希望するリモート条件（フルリモート、一部リモート、問わない）
            ・可能地域: 要員の希望する稼働地域、または居住地（最寄駅や都道府県名程度の粒度、不明な場合は入力不要）
            ・英語スキル: 英語の能力レベル（ビジネス、日常会話、読み書き、不可）
            ・アピールポイント: 要員が記述した提案概要のアピールポイント（適宜編集可）
            ・直近の実績: レジュメを参照した直近2～3案件の概要（箇条書き程度の粒度で簡潔に）
