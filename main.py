...
PROMPTS_emotional_Qgenerator = {
    "emotional_Qgenerator": {
        ...
        ❹ 回答が短い場合(50文字以下)は「もう少し詳しく教えていただけますか？」「具体的にはどのような...？」で詳細を促すこと。
...
PROMPTS_business_Qgenerator = {
    "business_Qgenerator": {
        ...
        ▼ルール
        1) カテゴリーの網羅
           - 上記「質問カテゴリー定義」を唯一の参照元とし、過去の質問と回答から既にカバー済みのカテゴリーを検出する。
           - 未収集カテゴリーを優先して1つ選び、重複質問は行わない（表現違いの重複も避ける）。

        2) 質問スタイル
           - 1メッセージ1論点。具体例・数値（期間/人数/件数/頻度/金額/割合/時期）を明示的に促す。
           - 曖昧な回答が想定される場合は、ひと呼吸で答えられる具体化プロンプト（「例えば…」）を1つだけ添える。

        3) 出力形式
           - 質問文のみを日本語で返す（前置き/箇条書き/装飾/要約/メタ説明は出力しない）。

        4) 終了条件
           - 全カテゴリーが収集できたと判断できる場合は、最後に「確認のための要約提示→合意確認→次のステップの短い案内」に関する1つの質問を生成して終了する。

        ▼補足
        - 「質問カテゴリー定義」は運用時に自由に編集してよい。名称は短く一意にする（10項目以内推奨）。
        - 過去の質問と回答は、与えられるコンテキスト（過去ログ）から読み取り、カバー状況を自律的に判断する。
""")}}
