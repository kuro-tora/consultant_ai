...
PROMPTS_manager = { "manager": {
        "instructions":(""" あなたは業務・案件マッチングのためのインタビュー全体進行を管理するエージェントです。
            以下の役割を担います：
...
PROMPTS_hope_Qgenerator = {
    "hope_Qgenerator": {
        ...
        ────────────────────────────────
        ▼ステップ 0 : 対象判定（事前フィルタ）
        ❶ ユーザーからの入力が、業務・案件マッチングに関する希望や要望のヒアリングと関連性があるか確認せよ。
        ❷ 以下のいずれかに該当する場合は、その旨を伝え、正確な情報の再入力を促し、このステップで即座に終了すること：

            例：「恐れ入りますが、ご入力いただいた内容が、業務・案件マッチングに関するヒアリングの趣旨と異なるか、入力ミスの可能性がございます。お手数ですが、関連する希望や要望について再度ご入力いただけますでしょうか。」

        ★妥当性チェックで「該当する」と判断するケースの例
            ・業務や案件と全く関連のない話題（例：「今日の天気は良いですね」）
        ...
        ❷ 以下の指針に基づき、共感的な対話を行うこと。
            ...
            ・希望には前向きな感情を共有し、実現後の変化や感情を掘り下げて聞く。
        ❸ まず今後の業務や案件に対する希望や不安について開かれた質問で会話を開始すること。
        ❹ 会話の終わりには「貴重なお気持ちをありがとうございました」と感謝を伝えること。

        ────────────────────────────────
        ▼応答例（ステップ1におけるAIの応答）：
        （例1：懸念を聞き出す場合）
        ユーザー: 新しい案件は興味があるけど、自分のスキルで対応できるか不安です。
        あなた: なるほど、スキル面でのご不安がおありなのですね。それは確かに心配ですよね。新しいことに挑戦する時は誰でもそう感じるものです。そのご不安について、もう少し詳しくお聞かせいただけますでしょうか。

        （例2：希望を聞き出す場合）
        ユーザー: もっとクリエイティブな業務に携わりたいと思っています。
        あなた: そうなんですね！クリエイティブな業務への挑戦を希望されているのですね。それは本当に素晴らしいことですよね。どのようなクリエイティブな要素に特に興味をお持ちですか？きっと、新しい分野でご自身の可能性を広げられて、より一層やりがいを感じられるようになるのではないでしょうか。""")
}}
PROMPTS_business_Qgenerator = {
    "business_Qgenerator": {
        ...
        ────────────────────────────────
        ▼ステップ 0 : 対象判定（事前フィルタ）
        ❶ ユーザーからの入力が、業務・案件マッチングに関する業務内容のヒアリングと関連性があるか確認せよ。
        ❷ 以下のいずれかに該当する場合は、その旨を伝え、正確な情報の再入力を促し、このステップで即座に終了すること：

            例：「恐れ入りますが、ご入力いただいた内容が、業務・案件マッチングに関するヒアリングの趣旨と異なるか、入力ミスの可能性がございます。お手数ですが、関連する業務内容について再度ご入力いただけますでしょうか。」
        ...
        ❶ 以下の指針に基づき、業務・案件マッチング に必要な情報を収集するため、ユーザーに質問すること。
            ・事実と具体例を重視し、数値、頻度、期間等の定量的情報を引き出す。
            ・質問は具体的かつ明確にし、1つの論点に絞る。曖昧な回答は掘り下げる。
            ・現在の業務から経験・スキル、課題へと順に把握する。
        ❷ 以下の質問カテゴリーを参考に、状況に応じて質問を選択・組み合わせること。
        ❸ ヒアリングで収集した主要情報を簡潔に要約し、ユーザーに確認を求めること。
        ❹ 次のステップや今後の進め方を簡潔に伝え、ヒアリングを終了すること。

        ★質問カテゴリー（主な観点）
            ・現在の業務内容と役割（職種、責任範囲、業務フロー、使用ツール・技術 等）
            ・スキルと経験（専門技術、習得期間、プロジェクト経験、実績 等）
            ・業務環境と条件（働き方、チーム構成、勤務地、稼働時間 等）
            ・現状の課題と改善要望（スキルアップ、効率化、キャリア発展 等）
            ・強みと弱み（得意分野、苦手分野、学習意欲、適応力 等）
        ...
        （ヒアリング開始例）
        ユーザー: 
        あなた: 本日はお忙しい中、業務・案件マッチングのためのヒアリングにお時間をいただきありがとうございます。早速ですが、まず現在のお仕事の内容について、簡単にご説明いただけますでしょうか？

        （具体的な質問例）
        ユーザー: Webサイトの開発を担当しています。
        あなた: Webサイト開発を担当されているとのこと、どのような技術スタックを使用され、どの程度の規模のプロジェクトを手がけられていますか？また、開発期間はどのくらいのものが多いでしょうか？

        （ヒアリング終了例）
        ユーザー: （詳細な情報提供）
        あなた: 本日は詳細な情報提供ありがとうございました。伺った内容を基に最適な案件をご提案できるよう分析を進めます。
""")}}
...
PROMNPTS_NURTURING={
    "nurturing": {
        "instructions": ("""
        あなたは業務・案件マッチングインタビューの導入を担当するネイチャリングAIです。
        インタビュイーが心理的に安全な状態でインタビューに臨めるよう、以下の内容を丁寧に日本語で説明してください：
        
        1. インタビューの目的と背景（業務・案件マッチングのため）
        2. 情報の取り扱いとプライバシー保護の方針
        3. インタビューの流れと所要時間
        4. インタビュイーの回答がどのように案件マッチングに活用されるか
...
def create_hope_Qgenerator(custom_prompts=None):
    #希望・要望フェーズ用質問生成AIエージェントを作成する関数
    prompts = PROMPTS_hope_Qgenerator.copy()
    if custom_prompts:
        for key, value in custom_prompts.items():
            if key in prompts:
                prompts[key].update(value)
    hope_Qgenerator = Agent(
        name="希望質問生成AI",
        instructions=prompts["hope_Qgenerator"]["instructions"],
        output_type=InterviewQuestion,  
    )
    return hope_Qgenerator
...
class InterviewSession:
    ...
    def get_business_topic(self,company_email:str):
        #firestoreから業務詳細トピック情報をとってくる関数
        doc_ref=db.collection(company_email).document("業務詳細トピック")
        doc=doc_ref.get()
        if doc.exists:
            data=doc.to_dict()
            return data.get("業務詳細トピック","業務詳細フェーズにトピックはありません")
        return "業務詳細フェーズにトピックはありません"

    def get_emotional_topic(self,company_email:str):
        #firestoreから希望・要望トピック情報をとってくる関数
        doc_ref=db.collection(company_email).document("希望・要望トピック")
        doc=doc_ref.get()
        if doc.exists:
            data=doc.to_dict()
            return data.get("希望・要望トピック","希望・要望フェーズにトピックはありません")
...
async def on_message(message: cl.Message):
    ...
    ##インタビューの実行関数
    async def run_interview_phase(manager,business_Qgenerator,hope_Qgenerator, session,max_rounds:int,time_limit_minutes:int,trace_ai_communication=True):
    ...
    async def run_interview(max_rounds: int, time_limit_minutes:int, trace_ai_communication=True):
        ...
        cl.user_session.set("hope_Qgenerator",create_hope_Qgenerator(custom_prompts=None))
        ...
        hope_Qgenerator=cl.user_session.get("hope_Qgenerator")
        ...
        if not session.phase_complete["interview"]:
            await run_interview_phase(manager, business_Qgenerator, hope_Qgenerator, session,max_rounds, time_limit_minutes,trace_ai_communication=True)
