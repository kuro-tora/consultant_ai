...
PROMPTS_manager = { "manager": {
        "instructions":(""" あなたは業務・案件マッチングのためのインタビュー全体進行を管理するエージェントです。
            ...
            4. 詳細誘導：回答が浅い場合は深掘りを促し、具体的な情報を引き出す

            インタビューは2つのフェーズに分かれています：
            - 経験・スキルフェーズ：現在の業務内容、経験、スキル、実績などの客観的情報を収集
            - 希望・要望フェーズ：今後の希望、要望、条件、懸念など、主観的な情報を収集

            インタビュー全体の流れを見て、次のアクションを決定してください：
            - deep_dive: 回答が浅い・抽象的・短い場合に同じトピックでさらに深掘りする
            - switch_topic: 現在のトピックが十分にカバーされた場合に特定のトピックへ移行する
            - switch_phase: 次のフェーズに移行する（経験・スキル→希望・要望）
            - end_interview: インタビューを終了する

            【深掘りを促す条件】
            - 回答が「はい」「いいえ」など単純すぎる場合
            - 具体的な例や数値が不足している場合
            - 経験の詳細や背景が不明な場合
            - スキルレベルや期間が曖昧な場合

            経験・スキルフェーズのトピックが十分にカバーできたら、希望・要望フェーズへの移行を指示してください。
...
PROMNPTS_NURTURING={
    "nurturing": {
        "instructions": ("""
        あなたは業務・案件マッチングのためのインタビュー導入を担当するAIです。
        インタビュイーが心理的に安全な状態でインタビューに臨めるよう、以下の内容を丁寧に日本語で説明してください：
        
        1. 業務・案件マッチングインタビューの目的と背景（最適な仕事の機会を見つけるため）
        2. 個人情報・経験情報の取り扱いとプライバシー保護の方針
        3. インタビューの流れと所要時間（希望・経験・スキルの順での質問）
        4. 収集した情報がどのように活用されるか（マッチング精度向上のため）
...
def create_business_Qgenerator(custom_prompts=None):
    #経験・スキルフェーズ用質問生成AIエージェントを作成する関数
...
class InterviewSession:
    def __init__(self, company_email: str):
        ...
        self.phases = ["経験・スキルフェーズ", "希望・要望フェーズ"]
        self.current_phase = self.phases[0]  # 最初は経験・スキルフェーズ
